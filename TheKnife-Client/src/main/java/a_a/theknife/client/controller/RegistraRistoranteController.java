...
public class RegistraRistoranteController{
    ...
    private void gestisciRegistrazione(ActionEvent event){
        try{
            ...
            if(rNome.isEmpty() || rIndirizzo.isEmpty() || rCitta.isEmpty() || rNazione.isEmpty() ||
               ...
               rRecapitoTelefonico.isEmpty() || rPaginaWeb.isEmpty() || rDescrizione.isEmpty() || rTipiCucina.isEmpty() || rServizi.isEmpty()){
                error("Campi vuoti", "Assicurarsi che tutti i campi contrassegnati dal carattere * siano non vuoti.\n"
                    + "Deve essere riportato almeno un tipo di cucina e un servizio");
                return; // impedisce il salvataggio di ristoranti sul db che non rispettano il vincolo di *
            }
            ...
            Ristorante ristorante = new Ristorante(rProprietario, rNome, rIndirizzo, rCitta, rNazione, rFasciaPrezzo, rLatitudine, rLongitudine, rRecapitoTelefonico, rPaginaMichelin, rPaginaWeb, rRiconoscimento, rStellaGreen, rDescrizione, rPrenotazioneOnline, rDelivery, rServizi, rTipiCucina);
