...
public class GestoreRecensioniImpl extends UnicastRemoteObject implements GestoreRecensioni{
    ...
    public ArrayList<Recensione> visualizzaRecensioni(Utente utente) throws RemoteException{
        String query = "SELECT re.id AS id_recensione, u1.username AS autore_recensione, re.valutazione, re.testo AS testo_recensione, re.ultima_modifica AS ultima_modifica_recensione, re.ristorante, ri.id AS id_risposta, u2.username AS autore_risposta, ri.testo AS testo_risposta, ri.ultima_modifica AS ultima_modifica_risposta "
                     + "FROM Recensioni re JOIN Utenti u1 ON re.autore = u1.id LEFT JOIN Risposte ri ON ri.recensione = re.id LEFT JOIN Utenti u2 ON u2.id = ri.autore "
                     + "WHERE re.autore = ?";
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            try(ResultSet resultSet = preparedStatement.executeQuery()){
                while(resultSet.next()){
                    ...
                    String autoreRecensione = resultSet.getString("autore_recensione");
                    ...
                    int ristorante = resultSet.getInt("ristorante");
                    ...
                    try{
                        recensioni.add(new Recensione(idRecensione, autoreRecensione, valutazione, testoRecensione, ultimaModificaRecensione, risposta, ristorante));
                    }
    ...
    public void scriviRecensione(Recensione recensione, int ristorante) throws RemoteException{
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            insertPreparedStatement.setInt(5, ristorante);
            
            insertPreparedStatement.executeUpdate();
        }
        catch(SQLException e){
            e.printStackTrace();
            throw(new RemoteException("Errore DB"));
        }
    }
