...
public class GestoreRistorantiImpl extends UnicastRemoteObject implements GestoreRistoranti{
    ...
    public ArrayList<Ristorante> visualizzaRistoranti(Utente utente) throws RemoteException{
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            throw(new RemoteException("Errore", e));
        }
        return(ristoranti);
    }
    ...
    public ArrayList<Recensione> visualizzaRecensioniPerRistorante(Ristorante ristorante) throws RemoteException{
        String query = "SELECT re.id AS id_recensione, re.autore AS autore_recensione, re.valutazione, re.testo AS testo_recensione, re.ultima_modifica AS ultima_modifica_recensione, ri.id AS id_risposta, ri.autore AS autore_risposta, ri.testo AS testo_risposta, ri.ultima_modifica AS ultima_modifica_risposta "
                     + "FROM Recensioni re LEFT JOIN Risposte ri ON re.id = ri.recensione "
                     + "WHERE re.ristorante = ? "
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            try(ResultSet resultSet = preparedStatement.executeQuery()){
                while(resultSet.next()){
                    ...
                    String autore = resultSet.getString("autore_recensione");
            ...
            throw(new RemoteException("Comunicazione col DB fallita", e));
        }
        return(recensioni);
    }
    ...
    public void apriRistorante(Ristorante ristorante) throws RemoteException, InvalidCoordinatesException{
        String ristoranteQuery = "INSERT INTO Ristoranti(proprietario, nome, indirizzo, citta, nazione, fascia_prezzo, latitudine, longitudine, recapito_telefonico, pagina_michelin, pagina_web, riconoscimento, stella_green, descrizione, prenotazione_online, delivery) "
                               + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                               + "RETURNING id";
        String serviziQuery = "INSERT INTO ServiziOfferti (ristorante, servizio) "
                            + "VALUES (?, ?)";
        String proposteCulinarieQuery = "INSERT INTO ProposteCulinarie(ristorante, tipo_cucina) "
                                      + "VALUES (?, ?)";
        ...
        try{
            ...
            try(PreparedStatement ristorantePreparedStatement = connection.prepareStatement(ristoranteQuery);
                PreparedStatement serviziPreparedStatement = connection.prepareStatement(serviziQuery);
                PreparedStatement proposteCulinariePreparedStatement = connection.prepareStatement(proposteCulinarieQuery)){
                ristorantePreparedStatement.setInt(1, ristorante.getProprietario());
                ristorantePreparedStatement.setString(2, ristorante.getNome());
                ristorantePreparedStatement.setString(3, ristorante.getIndirizzo());
                ristorantePreparedStatement.setString(4, ristorante.getCitta());
                ristorantePreparedStatement.setString(5, ristorante.getNazione());
                ristorantePreparedStatement.setString(6, ristorante.getFasciaPrezzo());
                ristorantePreparedStatement.setDouble(7, ristorante.getLatitudine());
                ristorantePreparedStatement.setDouble(8, ristorante.getLongitudine());
                ristorantePreparedStatement.setString(9, ristorante.getRecapitoTelefonico());
                ristorantePreparedStatement.setString(10, ristorante.getPaginaMichelin());
                ristorantePreparedStatement.setString(11, ristorante.getPaginaWeb());
                ristorantePreparedStatement.setString(12, ristorante.getRiconoscimento());
                ristorantePreparedStatement.setBoolean(13, ristorante.getStellaGreen());
                ristorantePreparedStatement.setString(14, ristorante.getDescrizione());
                ristorantePreparedStatement.setBoolean(15, ristorante.getPrenotazioneOnline());
                ristorantePreparedStatement.setBoolean(16, ristorante.getDelivery());
                
                int idRistorante;
                try(ResultSet resultSet = ristorantePreparedStatement.executeQuery()){
                    if(resultSet.next()){
                        idRistorante = resultSet.getInt("id");
                    }
                    else{
                    ...
                    throw(new RemoteException("Errore inserimento ristorante", e));
                }
                
                serviziPreparedStatement.setInt(1, idRistorante);
                for(int i = 0; i < ristorante.getServizi().size(); i++){
                    serviziPreparedStatement.setString(2, ristorante.getServizi().get(i));
                    serviziPreparedStatement.addBatch();
                }
                serviziPreparedStatement.executeBatch();
                
                proposteCulinariePreparedStatement.setInt(1, idRistorante);
                for(int i = 0; i < ristorante.getTipiCucina().size(); i++){
                    proposteCulinariePreparedStatement.setString(2, ristorante.getTipiCucina().get(i));
                    proposteCulinariePreparedStatement.addBatch();
                }
                proposteCulinariePreparedStatement.executeBatch();
            ...
            throw(new RemoteException("Errore apertura del ristorante", e));
        }
        finally{
            if(connection != null){
                try{
