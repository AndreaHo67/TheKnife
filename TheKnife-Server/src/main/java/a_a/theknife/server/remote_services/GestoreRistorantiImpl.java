...
public class GestoreRistorantiImpl extends UnicastRemoteObject implements GestoreRistoranti{
            ...
            try(ResultSet resultSet = preparedStatement.executeQuery()){
                ...
                while(resultSet.next()){
                    ...
                    if(id == idCorrente){
                        ...
                        String paginaMichelin = (resultSet.getObject("pagina_michelin", String.class) == null ? null : resultSet.getObject("pagina_michelin", String.class));
    ...
    public ArrayList<Ristorante> ricercaRistorantiPerFiltri(Filtro[] filtri) throws RemoteException{
        String query = "SELECT r.id, r.proprietario, r.nome, r.indirizzo, r.citta, r.nazione, r.fascia_prezzo, r.latitudine, r.longitudine, r.recapito_telefonico, r.pagina_michelin, r.pagina_web, r.riconoscimento, r.stella_green, r.descrizione, r.prenotazione_online, r.delivery, s.servizio, p.tipo_cucina "
                     ...
                     +                "WHERE r2.nome ILIKE(?) "
                     +                "AND r2.citta ILIKE(?) "
                     +                "AND r2.nazione ILIKE(?) "
                     +                "AND r2.riconoscimento ILIKE(?) ";
        ...
        String filtroTipoCucina = "AND EXISTS (SELECT 1 "
                                ...
                                +             "WHERE p2.tipo_cucina ILIKE(?) AND p2.ristorante = r2.id) ";
        
        String filtroServizio = "AND EXISTS (SELECT 1 "
                              +             "FROM ServiziOfferti s2 "
                              +             "WHERE s2.servizio ILIKE(?) AND s2.ristorante = r2.id) ";
    ...
    @Override // -- FUNZIONA --
    // (u_re.nome || ' ' || u_re.cognome): u:re.nome unito allo spazio vuoto, che viene unito al cognome u_re.cognome. Tutto rinominato come autore_recensione, in modo tale da avere su unica riga il nome completo dell'autore della recensione, da usare nella grafica 
    public ArrayList<Recensione> visualizzaRecensioniPerRistorante(Ristorante ristorante) throws RemoteException{
        String query = "SELECT re.id AS id_recensione, (u_re.nome || ' ' || u_re.cognome) AS autore_recensione, re.valutazione, re.testo AS testo_recensione, re.ultima_modifica AS ultima_modifica_recensione, re.ristorante, ri.id AS id_risposta, (u_ri.nome || ' ' || u_ri.cognome) AS autore_risposta, ri.testo AS testo_risposta, ri.ultima_modifica AS ultima_modifica_risposta "
                     + "FROM Recensioni re "
                     + "JOIN Utenti u_re ON re.autore = u_re.id "
                     + "LEFT JOIN Risposte ri ON re.id = ri.recensione "
                     + "LEFT JOIN Utenti u_ri ON ri.autore = u_ri.id "
                     + "WHERE re.ristorante = ? "
        ...
        try(Connection connection = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            try(ResultSet resultSet = preparedStatement.executeQuery()){
                while(resultSet.next()){
                    ...
                    int _ristorante = resultSet.getInt("ristorante");
                    ...
                    try{
                        recensioni.add(new Recensione(idRecensione, autore, valutazione, testoRecensione, ultimaModificaRecensione, risposta, _ristorante));
                    }
    ...
    public void apriRistorante(Ristorante ristorante) throws RemoteException, InvalidCoordinatesException{
        ...
        try{
            ...
            if(ristorante.getPaginaMichelin() == null){
                ristorantePS.setNull(10, java.sql.Types.VARCHAR);
            }
            else{
                ristorantePS.setString(10, ristorante.getPaginaMichelin());
            }
