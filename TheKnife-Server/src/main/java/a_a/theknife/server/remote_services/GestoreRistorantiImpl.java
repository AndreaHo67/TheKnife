...
public class GestoreRistorantiImpl extends UnicastRemoteObject implements GestoreRistoranti{
    ...
    public ArrayList<Recensione> visualizzaRecensioniPerRistorante(Ristorante ristorante) throws RemoteException{
        String query = "SELECT re.id AS id_recensione, (u_re.nome || ' ' || u_re.cognome) AS autore_recensione, re.valutazione, re.testo AS testo_recensione, re.ultima_modifica AS ultima_modifica_recensione, ri.id AS id_risposta, (u_ri.nome || ' ' || u_ri.cognome) AS autore_risposta, ri.testo AS testo_risposta, ri.ultima_modifica AS ultima_modifica_risposta "
                     + "FROM Recensioni re "
